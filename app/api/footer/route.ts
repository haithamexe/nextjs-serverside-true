...
export async function GET() {
  try {
    ...
    console.error("Error handling GET request on /api/footer:", error);
...
export async function POST(data: any) {
  try {
    ...
    console.error("Error handling POST request on /api/footer:", error);
    return NextResponse.json(
      { error: (error as Error)?.message || "Internal Server Error" },
      { status: 500 },
    );
...
export async function DELETE(data: any) {
  try {
    ...
    console.error("Error handling DELETE request on /api/footer:", error);
    return NextResponse.json(
      { error: (error as Error)?.message || "Internal Server Error" },
      { status: 500 },
    );
